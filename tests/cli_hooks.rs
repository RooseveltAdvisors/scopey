...
fn assert_clean_injection(stdout: &[u8]) {
    ...
    assert!(context.contains("For this task do not run tools"));
    assert!(context.contains("This is a read-only review"));
    assert!(!context.contains("scopey-user-constraint"));
    for phrase in [
        "CRITICAL:",
        "edit files",
